...
function mergeObjects(aObj) {
  ...
  aObj.forEach((o) => {
    const aKeys = o.keys();
    aKeys.forEach((k) => {
      if (k in oRes) oRes[k] += o[k];
      else oRes[k] = o[k];
    });
  });

  // aObj.array.forEach(element => {

  // });
  // for (const i in aObj) {
  //   for (const k in aObj[i]) {
  //     if (oRes.hasOwnProperty(k)) oRes[k] += aObj[i][k];
  //     else oRes[k] = aObj[i][k];
  //   }
  // }
...
function compareObjects(obj1, obj2) {
  const aKeys1 = Object.keys(obj1);
  const aKeys2 = Object.keys(obj2);
  ...
  let fl = true;
  aKeys1.forEach((k) => {
    if (!aKeys2.includes(k) || obj1[k] !== obj2[k]) fl = false;
  });
...
function makeWord(obj) {
  const aRes = [];
  const aLetters = Object.keys(obj);
  aLetters.forEach((v) => {
    obj[v].forEach((m) => {
      aRes[m] = v;
    });
...
function sellTickets(queue) {
  ...
  queue.forEach((v) => {
    switch (v) {
      ...
      case 100:
        if (q25 < 1) flOK = false;
        else if (q50 < 1 && q25 < 3) flOK = false;
        else {
          q25 -= 1;
          if (q50 >= 1) q50 -= 1;
          else q25 -= 2;
